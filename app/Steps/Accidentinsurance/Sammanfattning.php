...
use App\Mail\Accident as AccidentMail;
...
{
    public $name = 'olycksfallsforsakring-sammanfattning';
    ...
    {
        $date = $this->get_data('date') ?? now();
        $date = ($date instanceof Carbon)
            ? $date->toDateString()
            : Carbon::parse($date)->format('Y-m-d');

        return view('steps.accidentinsurance.sammanfattning', [
            'customer'         => $this->get_data('customer'),
            'ssn'              => $this->get_data('ssn'),
            'email'            => $this->get_data('email'),
            'phone'            => $this->get_data('phone'),
            'uppsagning'       => $this->get_data('uppsagning'),
            'date'             => $date,
            'betalningstermin' => $this->get_data('betalningstermin') ?? 12,
        ]);
    }
    ...
    public function validateStep(Request $request)
    {
        ...
        $this->store_data($input, 'details');
        $this->store_data($input, 'olycksfallsforsakring-sammanfattning');

        return response()->json([
            'status' => 1,
            'next_step' => 'olycksfallsforsakring-tack',
        ]);
    }
    ...
    public function bankid_status()
    {
        ...
        try {
            ...
            $data = (new FocusApi)->bankid_login_check($orderRef);
        ...
        if ($bankid_status == 'COMPLETE') {
            $focus_data = (new FocusApi)->get_shared_focus_data();
    ...
    public function send_focus_data()
    {
        ...
        $notes = 'Kunden har tecknat olycksfallsförsäkring via webben.';
        ...
        try {
            ...
            // Dunstan
            Mail::to($send_email_to)->send(new AccidentMail(
                'Nytecknad olycksfallsförsäkring via web',
                [
